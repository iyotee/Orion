...
if not exist "wiki_backup\Home.md" (
    echo [ERROR] Wiki files not found in wiki_backup directory. Please run this script from the documentation directory.
    pause
    exit /b 1
)
...
REM List of Wiki files to copy from wiki_backup
set WIKI_FILES=Home.md Installation-Guide.md Quick-Start.md Development-Guide.md Contributing-Guidelines.md Architecture-Overview.md Wiki-README.md _wiki.yml Wiki-Deployment-Guide.md Wiki-Summary.md TODO.md ROADMAP.md STORAGE_FRAMEWORK.md WIKI_UPDATE.md WIKI-COMPLETE.md

REM Copy each file from wiki_backup
for %%f in (%WIKI_FILES%) do (
    if exist "%SOURCE_DIR%\%%f" (
        copy "%SOURCE_DIR%\%%f" "%WIKI_DIR%\" >nul
        echo [INFO] Copied %%f
    ) else (
        echo [WARNING] File %%f not found in wiki_backup, skipping...
    )
)
...
if errorlevel 1 (
    ...
    REM Commit changes
    git commit -m "Complete ORION OS Wiki documentation - Added 15 comprehensive documentation pages with professional academic tone"
