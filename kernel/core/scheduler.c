...
static int copy_to_user(void* user_ptr, const void* kernel_data, size_t size) {
    ...
    // TODO: Add proper memory protection checks
    // For now, just do a simple memcpy (this is not safe in a real system)
    memcpy(user_ptr, kernel_data, size);
    return OR_OK;
}
...
// ========================================
// CONSTANTES ET MACROS CFS
// ========================================

#define SCHED_LATENCY_NS      6000000ULL    // 6ms latence cible
#define MIN_GRANULARITY_NS    750000ULL     // 0.75ms granularité minimum
#define NICE_0_WEIGHT         1024          // Poids pour nice 0
#define RB_RED                0
#define RB_BLACK              1

// Poids pour les différents niveaux de priorité (nice -20 à +19)
...
process_t* scheduler_create_process(void) {
    ...
    // Initialiser la table des handles
...
thread_t* scheduler_create_thread(process_t* process, uint64_t entry_point, uint64_t stack_pointer, uint64_t arg) {
    ...
    thread->cpu_affinity = 0xFFFFFFFFFFFFFFFFULL; // Toutes les CPUs
    
    // Configuration stack et registres
    thread->rsp = stack_pointer;
    thread->rip = entry_point;
    thread->rflags = 0x202; // IF set
    thread->registers[0] = arg; // RDI = premier argument
    
    // Allouer une stack kernel si nécessaire
    if (!stack_pointer) {
        ...
        thread->rsp = stack_vaddr + thread->stack_size - 8; // Top de la stack
    }
...
void scheduler_add_thread_to_rq(thread_t* thread) {
    ...
    // Mettre à jour le virtual runtime
    if (rq->rb_root) {
...
static thread_t* pick_next_thread(cpu_runqueue_t* rq) {
    ...
    // Trouver le thread le plus à gauche (plus petit virtual runtime)
...
static void remove_thread_from_rq(cpu_runqueue_t* rq, thread_t* thread) {
    // Retirer de l'arbre rouge-noir (implémentation simplifiée)
...
void scheduler_init(void) {
    ...
    // Initialiser les runqueues par CPU
...
void sched_yield(void) {
    ...
    // Si pas de thread à exécuter, idle
    if (!rq->current) {
        arch_cpu_idle();
    }
}
...
void scheduler_destroy_process(process_t* process) {
    ...
    while (thread) {
        ...
        // Retirer de la runqueue si nécessaire
        uint32_t cpu = arch_get_current_cpu();
        cpu_runqueue_t* rq = &runqueues[cpu];
...
void handle_cleanup(handle_t* handle) {
    ...
    switch (handle->type) {
        ...
        case HANDLE_TYPE_TIMER:
            ...
            // TODO: Implement timer cleanup when timer subsystem is ready
            kdebug("Timer handle cleanup not yet implemented");
            break;
        default:
            break;
    }
...
int elf_load_process(process_t* process, const char* path) {
    ...
    // TODO: Implement actual file system and ELF loading
