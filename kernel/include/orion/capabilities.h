...
// Types d'objets pour capabilities
typedef enum
{
    CAP_TYPE_NONE = 0,
    CAP_TYPE_MEMORY,
...
#define CAP_RIGHT_SEND (1ULL << 6)  // Pour IPC
#define CAP_RIGHT_RECV (1ULL << 7)  // Pour IPC
#define CAP_RIGHT_MAP (1ULL << 8)   // Pour mémoire
#define CAP_RIGHT_UNMAP (1ULL << 9) // Pour mémoire

// Fonctions du système de capabilities
...
// Vérifier les droits d'une capability
bool cap_check_rights(or_cap_t cap_id, uint64_t required_rights,
                      uint64_t caller_pid);

// Accorder/révoquer des droits
...
// Détruire une capability
void cap_destroy(or_cap_t cap_id);

// Macros pour les vérifications communes
